...
extension Color {
    ...
    static var secandaryText: Color {
...
extension View {
    func cornerRedius(_ radius :CGFloat,corner :UIRectCorner) -> some View {
        clipShape(RoundCorner(radius: radius, cornor: corner))
    }
}
struct RoundCorner :Shape {
    var radius :CGFloat = .infinity
    var cornor : UIRectCorner = .allCorners
    func path(in rect: CGRect) -> Path {
        let path = UIBezierPath(roundedRect: rect,byRoundingCorners: cornor,cornerRadii:  CGSize(width: radius, height: radius))
